...
public:
	...
	bool Cancel(Patient*& Cpatient, int Pcancel)
	{
		...
		while (ptr)
		{
			...
			if (ptr->getItem()->numberOfTreatments() == 1)
			{
				contain = true;
				break;
			}
		...
		if (randommm < Pcancel&& contain)
		{
			...
			if (frontPtr == backPtr)
			{
				if (frontPtr == nullptr)
					return false;
				if (frontPtr->getItem()->CurrentTreatment()->getType() == 'X' && frontPtr->getItem()->numberOfTreatments() == 1)
				{
					Cpatient = frontPtr->getItem();
					frontPtr = NULL;
					backPtr = frontPtr;
					Cpatient->setCancel(true);
					count--;
					return true;
				}
				...
				while (ptr != NULL)
				{
...

					{
						...
						if (ptr->getItem()->CurrentTreatment()->getType() == 'X' && ptr->getItem()->numberOfTreatments() == 1)
						{
